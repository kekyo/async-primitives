...
describe('Logical Context Hooks', () => {
  ...
  describe('setImmediate hook (Node.js only)', () => {
    it('should maintain logical context across setImmediate boundary when available', async () => {
      ...
      if (typeof globalThis.setImmediate === 'undefined') {
        console.log('Skipping setImmediate test - not available in this environment');
        return;
      }
    ...
    it('should handle setImmediate with arguments', async () => {
      if (typeof globalThis.setImmediate === 'undefined') {
        console.log('Skipping setImmediate with arguments test - not available in this environment');
        return;
      }
      ...
      setImmediate((arg1: string, arg2: number) => {
  ...
  describe('setTimeout hook', () => {
    ...
    it('should handle context switching within setTimeout boundary', async () => {
      ...
      const { setLogicalContextValue: freshSetValue, getLogicalContextValue: freshGetValue, switchToNewLogicalContext: freshSwitchToNewContext } = 
        await import('../src/primitives/logical-context');
  ...
  describe('Cross-hook integration', () => {
    it('should maintain context across mixed async operations', async () => {
      ...
      // queueMicrotask
      queueMicrotask(() => {
        results.push({ operation: 'queueMicrotask', value: freshGetValue(TEST_KEY) });
      });

      // setTimeout (already tested in existing tests)
      setTimeout(() => {
        results.push({ operation: 'setTimeout', value: freshGetValue(TEST_KEY) });
      }, 0);

      // setImmediate (if available)
      if (typeof globalThis.setImmediate !== 'undefined') {
        setImmediate(() => {
          results.push({ operation: 'setImmediate', value: freshGetValue(TEST_KEY) });
        });
      }

      // Promise.then
      Promise.resolve().then(() => {
        results.push({ operation: 'Promise.then', value: freshGetValue(TEST_KEY) });
      });
  ...
  describe('addEventListener hooks', () => {
    it('should have addEventListener hook implementation', () => {
      ...
      if (typeof Element !== 'undefined' && Element.prototype && Element.prototype.addEventListener) {
        ...
        console.log('addEventListener hooks not available in this environment - this is expected in some test environments');
      }
    });
  });
