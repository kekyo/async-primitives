...
import { runOnNewLogicalContext, switchToNewLogicalContext, getCurrentLogicalContextId } from '../src/primitives/logical-context';
...
describe('AsyncLocal', () => {
  ...
  describe('Async boundary behavior', () => {
    it('should maintain LogicalContext across setTimeout', async () => {
      ...
      const { createAsyncLocal } = await import('../src/primitives/async-local');
      const asyncLocal = createAsyncLocal<string>();
    ...
    it('should handle LogicalContext switching correctly across async boundaries', async () => {
      ...
      const { createAsyncLocal } = await import('../src/primitives/async-local');
      const { runOnNewLogicalContext } = await import('../src/primitives/logical-context');
      const asyncLocal = createAsyncLocal<string>();
      ...
      expect(valuesInTimeout).toEqual(['internal context value', 'external context value']);
    });
  });
